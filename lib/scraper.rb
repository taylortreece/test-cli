...
class S
    ...
    def c_maj_chords(user_input)
        c_maj = Nokogiri::HTML(open("http://www.piano-keyboard-guide.com/key-of-#{user_input}.html"))
    ...
    def all_scale_names
        @names = []
        @names << scrape_major_key_names
        @names << scrape_minor_key_names
        @names[0].map! {|a| a.split("Scale")}
        @names
    end
